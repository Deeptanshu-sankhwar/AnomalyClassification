...
cnn = Sequential()
cnn.add(Conv2D(80, kernel_size=3, activation='relu', input_shape=(dims, dims,3), padding='same'))
cnn.add(MaxPooling2D(2))
cnn.add(Conv2D(40, kernel_size=3, activation='relu', padding='same'))
cnn.add(MaxPooling2D(2))
cnn.add(Conv2D(20, kernel_size=3, activation='relu', padding='same'))
cnn.add(Conv2D(20, kernel_size=3, activation='relu', padding='same'))
cnn.add(MaxPooling2D(2))
cnn.add(Flatten())
    #cnn.summary()
...
rnn = Sequential()
rnn.add(GRU(80, return_sequences=True))
rnn.add(GRU(80))

dense = Sequential()
dense.add(Dense(80,activation='relu'))
dense.add(Dense(80,activation='relu'))
dense.add(Dense(3,activation='sigmoid'))
...
final_model.summary()
final_model.compile(loss='sparse_categorical_crossentropy', optimizer=adm, metrics=['accuracy'])
    #final_model.summary()
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig("elementary.png")
    
print('\n'+ ". accuracy : " + str(history.history.get('val_acc')[-1]*100) + ' %')
final_model.save('predictYO_multi' +  '.h5')  # creates a HDF5 file 
    
from keras.models import load_model
listData = os.listdir('test/')
random.shuffle(listData)
podel = load_model('predictYO_multi.h5')
for item in listData:
        pvid = 'test/' + item
        print(pvid)
        pvidcv = cv2.VideoCapture(pvid)
        pframe = pullFrame(pvidcv)
        pframe = pframe.reshape(-1, 100, 128, 128, 3)
        res = podel.predict(pframe)[0]
        print(res)
        if (np.argmax(res) == 2):
            print("Shoplifting")
