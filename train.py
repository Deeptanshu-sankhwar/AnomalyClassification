...
def setEpochSteps(path, batchSize):
    listShoplift = os.listdir(path + 'shoplift_examples' + '/')
    listnoShoplift = os.listdir(path + 'noshoplift_examples' + '/')
    sum = len(listShoplift) + len(listnoShoplift)
...
def generate_data(path, batch_size, currentFold, dataType):
    Dict = {}
    fileList = []
    folders = ['noshoplift_examples', 'shoplift_examples']
